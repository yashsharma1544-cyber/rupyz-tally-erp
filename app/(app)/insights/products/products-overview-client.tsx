...
import {
  Package, TrendingUp, TrendingDown, Minus, ChevronRight, Search, ArrowLeft,
} from "lucide-react";
...
export function ProductsOverviewClient({ data }: { data: ProductsOverviewData }) {
    ...
    <div className="min-h-screen bg-paper pb-10">
      <div className="max-w-5xl mx-auto px-3 py-4 lg:px-6 lg:py-6">
        ...
        <Link href="/insights" className="text-xs text-ink-muted hover:text-ink inline-flex items-center gap-1 mb-3">
          <ArrowLeft size={11}/> Insights
        </Link>

        <h1 className="text-lg lg:text-xl font-semibold leading-tight inline-flex items-center gap-2 mb-1">
  ...
  return (
    <tr className="hover:bg-paper-subtle/30 group">
      <td className="px-3 py-2.5">
        <Link href={`/insights/products/${p.product_id}`} className="font-medium hover:text-accent hover:underline">
          {p.product_name}
        </Link>
      </td>
      ...
      <td className="px-3 py-2.5">
        <Link
          href={`/insights/products/${p.product_id}`}
