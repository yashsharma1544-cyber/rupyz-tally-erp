...
import {
  Target, Loader2, RotateCcw, Check, AlertCircle, Save, RefreshCw, MapPin, Search,
} from "lucide-react";
import {
  loadTallyAllocation, saveTallyAllocation, type TallyAllocRow,
} from "./tally-target-actions";

type Group = { party_group: string; root_group: string; parties: number; total_kg: number };
...
export function TallyTargetsClient({
  ...
  const filteredGroups = useMemo(() => {
    ...
    return partyGroups.filter((g) => g.party_group.toLowerCase().includes(q));
  }, [partyGroups, search]);

  const load = useCallback(async (g: string, p: string) => {
    if (!g || !p) return;
    setLoading(true); setMsg(null);
    try {
      const res = await loadTallyAllocation(g, p);
  ...
  useEffect(() => { if (groupName && period) load(groupName, period); }, [groupName, period, load]);
  ...
  async function handleSave() {
    if (!groupName) { setMsg({ kind: "err", text: "Pick a beat first" }); return; }
    ...
    try {
      const res = await saveTallyAllocation({
        partyGroup: groupName, periodLabel: period.trim(), targetKg: target,
  ...
  async function handleRefresh() {
    ...
    try {
      ...
      if (!resp.ok) { setMsg({ kind: "err", text: json.error || "Refresh failed" }); }
        ...
        setMsg({ kind: "ok", text: `Refreshed — ${json.inserted} sales rows loaded from Google Sheet. Reselect a beat to see updated shares.` });
        if (groupName && period) load(groupName, period);
      }
    } catch (e) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap bg-paper-card border border-paper-line rounded-md px-3 py-2">
        <span className="text-xs text-ink-muted">
          Tally sales loaded: <strong className="tabular text-ink">{fmtKg(rowCount)}</strong> line-items
        </span>
        <button
      ...
      <div className="bg-paper-card border border-paper-line rounded-md p-3">
        ...
        <div className="flex flex-wrap gap-1.5 max-h-44 overflow-y-auto">
          ...
          {filteredGroups.length === 0 && <span className="text-sm text-ink-muted py-2">No groups match.</span>}
        </div>
      </div>
          ...
          <div className="p-4 space-y-4">
            <div className="flex flex-wrap items-end gap-3">
              ...
              <div>
                ...
                <input
                  type="number" min="0" step="1" value={targetKg}
                  onChange={(e) => setTargetKg(e.target.value)} placeholder="e.g. 5000"
              ...
              <button
                onClick={autoFill} disabled={loading || rows.length === 0}
            ...
            <div className="flex flex-wrap items-center gap-4 text-xs bg-paper-subtle/40 border border-paper-line rounded px-3 py-2">
              ...
              <span className="text-ink-subtle ml-auto">{rows.length} shops · all-time Tally history</span>
            </div>
                          ...
                          <tr key={r.party_name} className="hover:bg-paper-subtle/40">
                            ...
                            <td className="px-3 py-2 text-right">
                              <input
                                ...
                                onChange={(e) => updateShare(r.party_name, e.target.value)}
                                placeholder="0"
            ...
            <div className="flex justify-end">
              <button
                onClick={handleSave}
                disabled={saving || loading || target <= 0}
