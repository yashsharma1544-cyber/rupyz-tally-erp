...
import { TeamActivitySyncPanel } from "./team-activity-panel";
...
export default async function SettingsPage() {
  ...
  const [
    ...
    supabase.from("rupyz_session").select("org_id, username, expires_at, last_refreshed_at, last_team_activity_sync_at").maybeSingle(),
  ...
  const lastTeamActivityAt =
    (session as { last_team_activity_sync_at?: string | null } | null)?.last_team_activity_sync_at ?? null;
  return (
    <>
      ...
      <div className="p-3 sm:p-6 max-w-4xl space-y-4 sm:space-y-6">
        ...
        />
        <TeamActivitySyncPanel lastSyncAt={lastTeamActivityAt} />
        {backfillState && (
