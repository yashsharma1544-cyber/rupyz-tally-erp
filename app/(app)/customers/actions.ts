...
//
// Filters mirror the UI: search, beat, city, salesman, type, active.
//   beatId === null → unassigned beat only (undefined = no filter)
//   city  === null → no city set only (undefined = no filter)
// =============================================================================
export async function exportCustomersCsv(filters: {
  ...
  try {
    ...
    if (filters.city === null) q = q.is("city", null);
    else if (filters.city) q = q.ilike("city", filters.city); // exact city, case-insensitive
...
> {
  try {
    ...
    for (const [beatId, candidateIds] of idsByBeat.entries()) {
      ...
      if (error) {
        errors.push({ row: 0, reason: `DB update failed for beat ${beatId}: ${error.message}` });
        continue;
      }
      updated += (data ?? []).length;
    }
