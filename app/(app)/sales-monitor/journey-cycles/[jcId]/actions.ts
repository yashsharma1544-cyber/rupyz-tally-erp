...
/**
 * Persist a single salesman's plan for the given JC. The full set of entries
 * (one per day) is replaced atomically: delete-then-insert. Days with
 * beat_id=null are simply not inserted (i.e. the salesman is off that day).
 */
...
export async function copyFromOtherJc(
  ...
  try {
    ...
    revalidatePath(`/sales-monitor/journey-cycles/${targetJcId}`);
