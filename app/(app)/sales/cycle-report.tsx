...
export default function CycleReportButton() {
  const [level, setLevel] = useState<Level>('customer');
  ...
  async function run() {
    ...
    try {
      ...
      const beats = Array.from(new Set(rows.map((r) => beatOf(r.beat)))).sort();
      ...
      for (const r of rows) {
        ...
        const k = keyOf(beat, r.party_name, co);

        let row = map.get(k);
        if (!row) {
          row = {
            name: level === 'beat' ? beat : r.party_name,
            beat,
            company: co,
            priorCy: 0,
            priorLy: 0,
      ...
      // Fold weekly kg in
      for (const { beat, w } of okWeeks) {
        if (level === 'beat') {
          const row = map.get(beat);
          if (row) {
        ...
        for (const c of w.customers) {
          const co = companyOf(c.company ?? '');
          const k = `${co}||${c.party_name}`;
          const row = map.get(k) ?? map.get(`||${c.party_name}`);
          if (!row) continue;
          row.w1 += c.w1_kg;
          row.w2 += c.w2_kg;
          row.w3 += c.w3_kg;
      ...
      await exportXlsx({
        filename: `Cycle_Report_${level === 'beat' ? 'Beats' : 'Customers'}_JC${currentJc}_${stamp()}`,
        sheetName: `JC${currentJc} report`,
        title: `Cycle report · ${prevLabel} then JC${currentJc}`,
